...
import { marathonEvents, MarathonEvent } from './marathonData';

interface MarathonListPageProps {
  onBack: () => void;
  onSelectMarathon: (event: MarathonEvent) => void;
}

function MarathonListPage({ onBack, onSelectMarathon }: MarathonListPageProps) {
      ...
      <div className="w-[375px] bg-gray-50 min-h-screen flex flex-col relative">
        ...
        <div className="sticky top-0 bg-white z-20 px-6 pt-16 pb-6">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-2xl font-bold text-gray-900">选择演示赛事</h1>
          </div>
        </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-orange-800 leading-relaxed">
                以下是模拟的第三方马拉松赛事小程序。选择任一赛事进入付款页面，体验天气保障集成流程。
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 px-6 pb-8">
          <div className="space-y-4">
            {marathonEvents.map((event, index) => (
              <button
                key={event.id}
                onClick={() => onSelectMarathon(event)}
              ...
              >
                ...
                <div className="space-y-2.5">
                  ...
                  <div className="flex items-center gap-2 text-sm text-gray-700">
                    ...
                    <span>全程马拉松 {event.distance}</span>
                  </div>
                </div>

                <div className="mt-4 pt-4 border-t border-gray-200/60 flex items-center justify-between">
                  <span className="text-sm text-gray-600">报名费</span>
                  <span className="text-xl font-bold text-gray-900">¥{event.registrationFee}</span>
                </div>
              </button>
            ))}
          </div>
...
export default MarathonListPage;
